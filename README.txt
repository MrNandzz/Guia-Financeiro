...
SOBRE A ESCRITA CONTÁBIL NOS CAMPOS DE VALOR (AJUSTADO)
--------------------------------------------------------------
Os campos de valor (renda mensal, valor de parcela de conta fixa, valor
de parcela de aquisição, limite de cartão) agora usam a mesma lógica do
teclado da tela de lançar gasto/entrada: os dígitos entram pelos
centavos e vão empurrando o resto pra esquerda. Digitar "450" mostra
R$ 4,50; digitar "4000" mostra R$ 40,00; digitar mais dígitos continua
crescendo o valor. Fica igual em qualquer lugar do app, e centavos
funcionam normalmente.


SOBRE O TAMANHO DO CARTÃO DE CRÉDITO
------------------------------------------
O card visual do cartão (com a foto ou cor personalizada) ficou mais
compacto — ocupa menos espaço na tela, principalmente quando você tem
vários cartões cadastrados.


SOBRE O MEDIDOR DO PAINEL (REDESENHADO)
---------------------------------------------
O valor "disponível hoje" foi movido pra fora do centro do medidor e
ficou logo abaixo do arco, numa área que o ponteiro nunca alcança em
nenhum valor possível — matematicamente garantido, não é só visual.
Assim o ponteiro fica sempre livre pra se mover sem nunca ficar
escondido atrás do número.


SOBRE GASTOS NO CARTÃO DE CRÉDITO (CORRIGIDO)
----------------------------------------------------
Gastos vinculados a um cartão nunca mais descontam automaticamente do
"disponível hoje" do Painel — nem na hora, nem quando a fatura "vence".
Eles são só uma dívida futura do cartão, controlada inteiramente na aba
Cartões (limite/usado) e na sub-aba Cartões dentro de Lançamentos, onde
cada cartão mostra tudo que está vinculado a ele agrupado. O saldo do
mês só é afetado por gastos sem cartão (débito/dinheiro), contas fixas
marcadas como pagas e parcelas de aquisições marcadas como pagas.


SOBRE A ABA LANÇAMENTOS (CONTAS + COMPRAS + GASTOS + CARTÕES)
---------------------------------------------------------------------
A barra de navegação foi de 6 para 4 ícones. Contas fixas, Aquisições e
Gastos/Entradas agora moram todos dentro de uma única aba "Lançamentos",
com abas internas: Contas, Compras, Gastos e Cartões (esta última mostra
cada cartão cadastrado com tudo que está vinculado a ele — gastos,
contas e parcelas de compras — agrupado e somado).


SOBRE AS ANIMAÇÕES NO IPHONE (CORRIGIDO)
-----------------------------------------------
As animações de toque não estavam funcionando por um comportamento
conhecido do Safari no iOS: ele só ativa o efeito visual :active ao
toque se existir algum listener de touchstart no documento. Sem isso,
mesmo com o CSS certo, nada acontecia visualmente ao tocar. Foi
adicionado esse listener, então agora todo botão, chip e card reage ao
toque também no iPhone. A vibração tátil continua limitada a
Android/Chrome — o Safari não implementa essa API do navegador.


SOBRE A ESCRITA CONTÁBIL NOS CAMPOS DE VALOR
---------------------------------------------------
Campos de valor (renda mensal, valor de parcela de conta fixa, valor de
parcela de aquisição, limite de cartão) agora formatam em tempo real
conforme você digita: escrever "4000" mostra "R$ 4.000,00" na hora,
sem precisar digitar pontos ou vírgulas. Esses campos trabalham em reais
inteiros (sem centavos) — se precisar lançar um valor com centavos
exatos, use o teclado numérico da tela de lançar gasto/entrada (o botão
+), que continua aceitando centavos normalmente.


SOBRE GASTOS NO CARTÃO DE CRÉDITO
---------------------------------------
Ao vincular um gasto avulso a um cartão, ele deixa de descontar na hora
do "disponível hoje" do Painel (ver correção acima). A aba Lançamentos →
Gastos continua mostrando todas as compras pela data real (pra você não
perder o controle de quando comprou o quê) — ela só não soma mais os
gastos de cartão no total que afeta seu saldo. Cada gasto de cartão
mostra uma notinha indicando em qual fatura ele vai cair.
...
----------------------------------------
Cada conta fixa, em Lançamentos → Contas, tem seus próprios ícones de
editar (lápis) e apagar (lixeira) na própria linha — não é mais preciso
entrar em Ajustes pra isso.
